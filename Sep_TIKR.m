function Sep_TIKR(beta,MicPos,SorPos)
...
%                 A(m,ss) =exp(-1j*k*r)/r;
                 ...
                 Source_half(:,ff,FrameNo)=inv(A'*A+(beta^2)*eye(size(A,2)))*A'*P_half(:,ff,FrameNo);
        end
        for ss = 1:SorNum
    ...
    for ss = 1:SorNum
        audiowrite(['TIKR_sep' num2str(MicNum) num2str(ss) '_' num2str(beta) '.wav'],p(ss,:)/max(abs(p(ss,:))),fs);
    end
end